...
public class PieceTrickSiphonAura extends PieceTrick {
    ...
    public Object execute(SpellContext context) throws SpellRuntimeException {
        if(context.caster.worldObj.isRemote || context.caster == null)
            return null;
        PlayerData data;
        ...
        for(Aspect aspect : primals){
            energy += AuraHelper.drainAuraAvailable(context.caster.worldObj, context.caster.getPosition(), aspect, 2 * this.<Double>getParamValue(context, power).intValue());
        }
        if(energy <= 0)
            return null;
